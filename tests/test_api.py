...
class TestFleetAPI:
    ...
    def test_get_fleet_overview_success(self, test_client):
        ...
        )  # Service olmadığı için 500 olabilir
...
class TestActionsAPI:
    ...
    def test_execute_bulk_actions_empty_sim_list(self, test_client):
        ...
        empty_request = {"sim_ids": [], "action": "freeze_24h", "reason": "test"}
...
class TestWhatIfAPI:
    ...
    def test_whatif_simulation_success(
        self, test_client, sample_sim_id, sample_whatif_request
    ):
        """Maliyet simülasyonu başarılı yanıt testi"""
        response = test_client.post(
            f"/api/v1/whatif/{sample_sim_id}", json=sample_whatif_request
        )
    ...
    def test_whatif_scenario_based_simulation(self, test_client, sample_sim_id):
        """Senaryo tabanlı simülasyon testi"""
        scenarios = [
            {"scenario": "increase_20", "parameters": {"duration_days": 30}},
            {"scenario": "decrease_30", "parameters": {"duration_days": 30}},
            {"scenario": "spike_day", "parameters": {"spike_multiplier": 10}},
            {"scenario": "roaming_week", "parameters": {"roaming_days": 7}},
        ]

        for scenario_data in scenarios:
            response = test_client.post(
                f"/api/v1/whatif/{sample_sim_id}", json=scenario_data
            )

            # Senaryo tabanlı simülasyon desteklenmeli
            assert response.status_code in [200, 404, 422, 500]

            if response.status_code == 200:
                data = response.json()
                # Scenario specific validation
                if scenario_data["scenario"] == "increase_20":
                    # %20 artış simülasyonu
                    assert (
                        "increase" in data.get("description", "").lower()
                        or data.get("candidate_total", 0) > 0
                    )
                elif scenario_data["scenario"] == "roaming_week":
                    # Roaming simülasyonu
                    assert (
                        "roaming" in data.get("description", "").lower()
                        or data.get("candidate_total", 0) > 0
                    )
    ...
    def test_whatif_only_plan_change(self, test_client, sample_sim_id):
        ...
        response = test_client.post(
            f"/api/v1/whatif/{sample_sim_id}", json=plan_only_request
        )
    ...
    def test_whatif_only_addons(self, test_client, sample_sim_id):
        ...
        response = test_client.post(
            f"/api/v1/whatif/{sample_sim_id}", json=addon_only_request
        )
...
class TestAPIIntegration:
    ...
    def test_post_endpoints_accessible(self, test_client):
        ...
        endpoints = ["/api/v1/analyze/2001", "/api/v1/whatif/2001", "/api/v1/actions"]
