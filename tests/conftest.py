...
TEST_USAGE_DATA = [
    {"timestamp": datetime.now(), "mb_used": 480.0, "roaming_mb": 0.0},
    {"timestamp": datetime.now(), "mb_used": 410.0, "roaming_mb": 0.0},
    {"timestamp": datetime.now(), "mb_used": 12.0, "roaming_mb": 0.0},
]
...
async def async_client():
    ...
    async with AsyncClient(
        app=test_app, base_url="http://test", follow_redirects=False
    ) as ac:
        yield ac
...
def mock_services():
    """Servisleri mock'la"""
    with patch("Public.API.v1.Libs.iot_service.iot_service") as mock_iot_service, patch(
        "Public.API.v1.Libs.anomaly_detector.anomaly_detector"
    ) as mock_anomaly, patch(
        "Public.API.v1.Libs.cost_simulator.cost_simulator"
    ) as mock_cost, patch(
        "DB.db_manager"
    ) as mock_db:
...
def sample_whatif_request():
    ...
    return {"scenario": "increase_20", "parameters": {"duration_days": 30}}


@pytest.fixture
def sample_scenario_requests():
    """Test scenario requests fixture"""
    return [
