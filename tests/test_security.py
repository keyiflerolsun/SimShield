...
class TestAPISecurity:
    ...
    def test_xss_protection(self, test_client):
        ...
        response = test_client.post(
            "/api/v1/actions",
            json={"sim_ids": ["2001"], "action": "freeze_24h", "reason": xss_payload},
        )
    ...
    def test_large_payload_protection(self, test_client):
        ...
        response = test_client.post(
            "/api/v1/actions",
            json={"sim_ids": large_sim_list, "action": "freeze_24h", "reason": "test"},
        )
...
class TestAPIAuthentication:
    ...
    def test_unauthorized_access(self, test_client):
        ...
        assert response.status_code in [
            200,
            500,
        ]  # 200 OK veya 500 server error kabul edilebilir
...
class TestAPIInputValidation:
    ...
    def test_required_fields_validation(self, test_client):
        ...
        # Eksik sim_ids
        response = test_client.post(
            "/api/v1/actions", json={"action": "freeze_24h", "reason": "test"}
        )
        ...
        # Eksik action
        response = test_client.post(
            "/api/v1/actions", json={"sim_ids": ["2001"], "reason": "test"}
        )
    ...
    def test_json_injection_protection(self, test_client):
        """JSON injection koruması testi"""
        malicious_json = {
            "sim_ids": ["2001"],
            "action": "freeze_24h",
            "reason": '{"malicious": "injection", "admin": true}',
        }

        response = test_client.post("/api/v1/actions", json=malicious_json)

        # JSON injection güvenli şekilde işlenmeli
        assert response.status_code in [200, 201, 422, 500]

        if response.status_code in [200, 201]:
            # Response'da injection payload'ı olmamalı
            assert '"admin":true' not in response.text.lower()

    def test_header_injection_protection(self, test_client):
        """Header injection koruması testi"""
        malicious_headers = {
            "X-Forwarded-For": "127.0.0.1\r\nX-Admin: true",
            "User-Agent": "Mozilla/5.0\r\nX-Inject: malicious",
        }

        response = test_client.get("/api/v1/fleet", headers=malicious_headers)

        # Header injection çalışmamalı
        assert response.status_code in [200, 400, 500]

        # Response header'larında injection olmamalı
        response_headers = dict(response.headers)
        assert "X-Admin" not in response_headers
        assert "X-Inject" not in response_headers
